...
public class SparkDataReader {
	...
	public static void main(String[] argv) {
		...
		hadoopConfiguration.set("es.nodes", "<ES_HOST>:9200");
		hadoopConfiguration.set("es.resource", "jenkins/buildlog");

		JavaPairRDD<Text, MapWritable> esRDD = sc.newAPIHadoopRDD(
				hadoopConfiguration, EsInputFormat.class, Text.class,
				MapWritable.class);
		...
		// city in the record
		JavaPairRDD<String, Integer> jobCountMap = esRDD
				.mapToPair(new PairFunction<Tuple2<Text, MapWritable>, String, Integer>() {
					public Tuple2<String, Integer> call(
							Tuple2<Text, MapWritable> currentEntry)
							throws Exception {
						
						MapWritable valueMap = currentEntry._2();
						//WritableArrayWritable address = (WritableArrayWritable) valueMap
								//.get(new Text("address"));
						//MapWritable addressMap = (MapWritable) address.get()[0];
						
						Text jobData = (Text) valueMap.get(new Text("jobName"));
						if(jobData != null) {
						 return new Tuple2<String, Integer>(jobData.toString(), 1);
						}else{
							return new Tuple2<String, Integer>("NA", 1);
						}
					}
				});

		// This is reducer which will maintain running count of city vs count
		JavaPairRDD<String, Integer> jobCount = jobCountMap
		...
		jobCount.saveAsTextFile("output/a3");
